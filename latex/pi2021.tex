...
\begin{frame}
  ...
  \uncover<2->{Zauważmy, że}
  \begin{enumerate}[label=\enumsymbol]
    \item<2-> \colora{28} zawiera się w $2^7 = 1\colora{28}$,
    \item<3-> \colora{314} zawiera się w $2^{74} = 188894659\colora{314}78580854784$,
    \item<4-> \colora{31415} zawiera się w $2^{144} = 2230074519853062\colora{31415}35718272648361505980416$,
    \item<5-> 70000000 zawiera się w $2^{9452}$,
    \item<6-> 2000000000 zawiera się w $2^{100824}$.
  \end{enumerate}
  \uncover<7->{Czy prawdą jest, że dla dowolnej liczby $n \in \N$ istnieje taki wykładnik $w \in \N$, że liczba $2^w$ zawiera liczbę $n$?}
\end{frame}
...
\begin{frame}
  ...
  \end{equation*}
  \begin{enumerate}[label=\enumsymbol]
    \item<2-> $\log_{10}100 = 2$, bo $10^2 = 100$,
    \item<3-> $\log_{10}10^\alpha = \alpha \quad \paren{\alpha \in \R}$,
    \item<4-> $\log_{10}2 = 0{,}30102\ldots{}, \log_{10}3 = 0{,}47712\ldots{}, \log_{10}4 = 0{,}60205\ldots{} \uncover<5->{\stackrel{\text{Lemat~\ref{lemma::log_10_rational_iff}}}{\in} \R \setminus \Q$,
    \item<6-> $10^{\log_{10}\alpha} = \alpha \quad \paren{\alpha \in \R}$,}
  \end{enumerate}
  \uncover<7->{\begin{equation} \label{eq::log_is_inverse_to_exp}
    10^{\log_{10}p^w} = p^w \quad \paren{p, w \in \N},
  \end{equation}}
  \uncover<8->{\begin{equation} \label{eq::exp_of_sum}
     10^{w_1+w_2} = 10^{w_1} \cdot 10^{w_2} \quad \paren{w_1, w_2 \in \R},
  \end{equation}}
  \begin{enumerate}[label=\enumsymbol]
    \item<9-> $\log_{10} x_1 x_2 = \log_{10}x_1+\log_{10}x_2 \quad \paren{x_1, x_2 > 0}$,
  \end{enumerate}
  \uncover<10->{\begin{equation} \label{eq::log_of_pow}
...
\begin{frame}
  \begin{proof}[Dowód lematu~\ref{lemma::powers_begin_with_every_string}]
    \begin{align*}
      \uncover<2->{p^w} & \uncover<3->{\stackrel{\eqref{eq::log_is_inverse_to_exp}}{=} 10^{\log_{10}p^w}}
            \uncover<4->{\stackrel{\eqref{eq::log_of_pow}}{=} 10^{w \log_{10} p}}
            \uncover<5->{\stackrel{\eqref{eq::frac_part_definition}}{=} 10^{\floor{w \log_{10} p} + \fracpart{w \log_{10} p}}} \\
          & \uncover<6->{\stackrel{\eqref{eq::exp_of_sum}}{=} \underbrace{10^{\floor{w \log_{10} p}}}_{10\ldots 0}\cdot10^{\fracpart{w \log_{10} p}}.}
    \end{align*}
    \begin{enumerate}[label=\enumsymbol]
      \item<7-> $10^{\log_{10} 1{,}2345} = 1{,}2345$,
      \item<8-> $10^{\log_{10} c_1{,}c_2c_3c_3 \ldots{} c_l} = c_1{,}c_2c_3c_3 \ldots{} c_l$.
    \end{enumerate}
    \uncover<9->{Niech $n = c_1c_2c_3 \ldots{} c_l$,}
    \uncover<10->{wobec twierdzenia~\ref{thm::fractional_part_of_multiples_of_irrational_numers_are_dense} istnieje $w > \frac{l}{\log_{10}p}$ takie, że
...
\begin{frame}
  \begin{theorem}
    Jeżeli ${\fracpart{\log_{10} a_k}}_{k \in \N}$ jest gęsty w przedziale  $[0, 1]$, to
    ciąg ${\paren{a_k}}_{k \in \N}$ \define{zawiera} wszystkie liczby.
  \end{theorem}
  \uncover<2->{
  \begin{example}
  \begin{enumerate}[label=\enumsymbol]
    \item<2-> $\paren{\fracpart{n\log_{10}2}}_{n \in \N} \rightsquigarrow \paren{2^n}_{n \in \N}$,
    \item<3-> $\paren{\fracpart{n^k\log_{10}2}}_{n \in \N} \rightsquigarrow \paren{2^{n^k}}_{n \in \N}$,
    \item<4-> $\paren{\fracpart{p_n\log_{10}2}}_{n \in \N} \rightsquigarrow \paren{2^{p_n}}_{n \in \N}$, gdzie $p_n$ oznacza $n$-tą liczbę pierwszą (zob.~\cite{Vinogradov1948}),
    \item<5-> $\paren{\fracpart{k\log_{10}n}}_{n \in \N} \rightsquigarrow \paren{n^k}_{n \in \N}$, dla dowolnego $k \in \N$,
    \item<6-> $\paren{\fracpart{\log_{10}n!}}_{n \in \N} \rightsquigarrow\paren{n!}_{n \in \N}$ (zob.~\cite{Diaconis1977}),
  \end{enumerate}
  \end{example}}
\end{frame}
