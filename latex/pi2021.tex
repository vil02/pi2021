...
\begin{frame}
  ...
  \end{equation*}
  \begin{enumerate}[label=$\mathbf{P}_{\arabic*}$]
    \item[\enumsymbol] $\log_{10}100 = 2$, bo $10^2 = 100$,
    \item[\enumsymbol] $\log_{10}10^\alpha = \alpha \quad \paren{\alpha \in \R}$,
    \item[\enumsymbol] $\log_{10}2 = 0{,}30102\ldots{}, \log_{10}3 = 0{,}47712\ldots{}, \log_{10}4 = 0{,}60205\ldots{}  \stackrel{\text{Lemat~\ref{lemma::log_10_irrational_iff}}}{\in} \R \setminus \Q$,
    \item[\enumsymbol] $10^{\log_{10}\alpha} = \alpha \quad \paren{\alpha \in \R}$,
    \item\label{eq::log_is_inverse_to_exp} $10^{\log_{10}p^w} = p^w \quad \paren{p, w \in \N}$,
    \item\label{eq::exp_of_sum} $10^{w_1+w_2} = 10^{w_1} \cdot 10^{w_2} \quad \paren{w_1, w_2 \in \R}$,
    \item[\enumsymbol] $\log_{10} x_1 x_2 = \log_{10}x_1+\log_{10}x_2 \quad \paren{x_1, x_2 > 0}$,
    \item\label{eq::log_of_pow} $\log_{10}{p^w} = w \log_{10}p  \quad \paren{w_1, w_2 \in \R}$
  \end{enumerate}
\end{frame}
...
\subsection{\texorpdfstring{$n! = 1\cdot2\cdot \ldots \cdot n, \quad \paren{n \in \N}$}{Silnia}}
