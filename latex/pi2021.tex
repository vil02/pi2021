...
\begin{frame}
  \begin{theorem}
    ...
    ciąg ${a}_{{k}_0}$ \define{zawiera} wszystkie liczby.
  \end{theorem}
  \begin{example}
  \begin{enumerate}[label=\enumsymbol]
    \item $\paren{2^{n^k}}_{n \in \N}$, gdzie $k \in \N$ (zob.~\cite{Weyl1916}),
    \item $\paren{2^{p_n}}_{n \in \N}$, gdzie $p_n$ oznacza $n$-tą liczbę pierwszą (zob.~\cite{Vinogradov1948}),
    \item $\paren{n^k}_{n \in \N}$, gdzie $k \in \N$,
