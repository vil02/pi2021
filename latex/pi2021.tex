...
\begin{frame}
  \begin{proof}[Dowód lematu~\ref{lemma::powers_begin_with_every_string}]
    \begin{align*}
      p^w & \stackrel{\eqref{eq::log_is_inverse_to_exp}}{=} 10^{\log_{10}p^w} \stackrel{\eqref{eq::log_of_pow}}{=} 10^{w \log_{10} p} \stackrel{\eqref{eq::frac_part_definition}}{=} 10^{\floor{w \log_{10} p} + \fracpart{w \log_{10} p}} \\
          &\stackrel{\eqref{eq::exp_of_sum}}{=} \underbrace{10^{\floor{w \log_{10} p}}}_{10\ldots 0}\cdot10^{\fracpart{w \log_{10} p}}.
    \end{align*}
    \begin{enumerate}[label=\enumsymbol]
